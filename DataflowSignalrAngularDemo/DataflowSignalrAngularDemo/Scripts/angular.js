 ...
 *   Given `<monitor my-attr="hello {{name}}">` and monitor definition
 *   of `scope: { localName:'@myAttr' }`, then monitor scope property `localName` will reflect
 *   the interpolated value of `hello {{name}}`. As the `name` attribute changes so will the
 *   `localName` property on the monitor scope. The `name` is read from the parent scope (not
 *   component scope).
 *
 * * `=` or `=attr` - set up bi-directional binding between a local scope property and the
 *   parent scope property of name defined via the value of the `attr` attribute. If no `attr`
 *   name is specified then the attribute name is assumed to be the same as the local name.
 *   Given `<monitor my-attr="parentModel">` and monitor definition of
 *   `scope: { localModel:'=myAttr' }`, then monitor scope property `localModel` will reflect the
 ...
 *   local name. Given `<monitor my-attr="count = count + value">` and monitor definition of
 ...
 * transclusion function which is pre-bound to the correct scope. In a typical setup the monitor
 * creates an `isolate` scope, but the transclusion is not a child, but a sibling of the `isolate`
 * scope. This makes it possible for the monitor to have private state, and the transclusion to
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  ...
  this.$get = [
     ...
     *        needed so that the jqLite collection items can be replaced with monitors.
...
function $RootScopeProvider(){
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
    ...
    Scope.prototype = {
       ...
       *         When creating monitors, it is useful for the monitor to not accidentally read parent
       *         state.
       *
 ...
 * <example module="customControl">
    ...
    <file name="index.html">
      <form name="myForm">
       <div contenteditable
            name="mymonitor" ng-model="userContent"
            strip-br="true"
            required>Change me!</div>
        <span ng-show="myForm.mymonitor.$error.required">Required!</span>
       <hr>
